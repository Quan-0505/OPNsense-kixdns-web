...
**引擎较新时多出的指标。** 当引擎的观察者接口提供额外字段时（插件按能力探测，老引擎下这些项不会出现）：

| 新增指标 | 含义 |
| --- | --- |
| 合并请求 | 与另一个相同查询合并、搭了在途请求便车的请求 —— 此前这类请求在统计中不可见 |
| 缓存来源 | 缓存中的答案由**哪个上游**提供 |
| 应答均字节 | 回给客户端的平均应答大小 |
| 客户端 RCODE | **实际回给客户端**的响应码（含缓存命中与规则合成的应答，不再只有转发口径） |

上游面板的占比只统计**真正服务了查询的上游**；并发扇出中落败的兄弟（`Aborted`）与未成功派发的尝试
（`Rejected`/`Error`）单独归入 outcomes，避免同一个上游被重复计数。
